(*---------------------------------------------------------------------------
  Copyright (c) 2013 The tsdl programmers. All rights reserved. Distributed
...
let pp_rect ppf r =
  pp ppf "@[<1><rect (%d %d) (%d %d)>@]" (Sdl.Rect.x r) (Sdl.Rect.y r)
    (Sdl.Rect.w r) (Sdl.Rect.h r)

let pp_color ppf c =
  pp ppf "@[<1><color %d %d %d %d>@]" (Sdl.Color.r c) (Sdl.Color.g c)
    (Sdl.Color.b c) (Sdl.Color.a c)

let pp_render_info ppf i =
  pp ppf "@[<v>@[%s@]@,%a@,@[max tex size %dx%d@]@]" i.Sdl.ri_name
    (pp_list Format.pp_print_string)
    (List.map Sdl.get_pixel_format_name i.Sdl.ri_texture_formats)
    i.Sdl.ri_max_texture_width i.Sdl.ri_max_texture_height
...
let pp_display_mode ppf m =
  pp ppf "@[<1>format:%s@ %dx%d@ @@ %a@]"
    ...
    m.Sdl.dm_w m.Sdl.dm_h (pp_unknown pp_hz) m.Sdl.dm_refresh_rate
...
let pp_controller_device_event ppf e =
  pp ppf "@[<1>controller_device_event %s which:%ld@ @]"
    Sdl.Event.(
      if get e typ = controller_device_added then "add"
      else if get e typ = controller_device_remapped then "remap"
      else if get e typ = controller_device_removed then "rem"
      else assert false)
    Sdl.Event.(get e controller_device_which)

let pp_dollar_gesture_event ppf e =
  pp ppf
    "@[<1>dollar_gesture_event touch_id:%Ld@ gesture_id:%Ld@ num_fingers:%d@ \
     error:%g@ (%g,%g)@]"
...
let pp_drop_event ppf e =
  pp ppf "@[<1>drop_event file:%a@]" (pp_opt pp_str)
    Sdl.Event.(drop_file_file e)

let pp_touch_finger_event ppf e =
  pp ppf
    "@[<1>touch_finger_event %s touch_id:%Ld@ finger_id:%Ld@ (%g,%g)@ \
     rel:(%g,%g)@ pressure:%g"
    Sdl.Event.(
      if get e typ = finger_down then "down"
      else if get e typ = finger_motion then "motion"
      else if get e typ = finger_up then "up"
      else assert false)
...
let pp_keyboard_event ppf e =
  pp ppf
    "@[<1>keyboard_event@ window_id:%d@ state:%s@ repeat:%b@ scancode:%s@ \
     keycode:%s@ keymod:%d@]"
...
let pp_mouse_button_event ppf e =
  pp ppf
    "@[<1>mouse_button_event window_id:%d@ which:%ld@ button:%d@ state:%s@ \
     (%d,%d)@]"
...
let pp_mouse_motion_event ppf e =
  pp ppf
    "@[<1>mouse_motion_event window_id:%d@ which:%ld@ state:%ld@ (%d,%d)@ \
     rel:(%d,%d)@]"
...
let pp_mouse_wheel_direction ppf x =
  if x = Sdl.Event.mouse_wheel_normal then pp ppf "normal"
  else if x = Sdl.Event.mouse_wheel_flipped then pp ppf "flipped"
  else assert false
...
let pp_mouse_wheel_event ppf e =
  pp ppf "@[<1>mouse_wheel_event window_id:%d@ which:%ld@ (%d,%d) %a @]"
    ...
    pp_mouse_wheel_direction
    Sdl.Event.(get e mouse_wheel_direction)

let pp_multi_gesture_event ppf e =
  pp ppf
    "@[<1>multi_gesture_event touch_id:%Ld@ dtheta:%f@ ddist:%f@ (%f,%f)@ \
     num_fingers:%d@]"
  ...
  pp ppf "%s"
    (List.assoc ty
       [
         ...
         (guitar, "guitar");
         (drum_kit, "drum_kit");
         (arcade_pad, "arcade_pad");
         (throttle, "throttle");
       ])

(*---------------------------------------------------------------------------
  Copyright (c) 2013 The tsdl programmers Permission to use, copy, modify,
