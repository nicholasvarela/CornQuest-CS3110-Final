...
let rec listen adv st =
  ...
  try
    ...
    | Go dir -> (
        match State.go (Game.Game_fmts.dir_to_string dir) adv st with
        | Legal lg ->
            let new_room = State.current_room_id lg in
            let new_room_desc = Adventure.description adv new_room in
            if List.mem new_room (State.visited st) then
              print_endline ("This place seems familiar. " ^ new_room_desc)
            else print_endline new_room_desc;
            listen adv lg
        | Illegal ->
            prompt
              ("You cannot exit this room from the "
              ^ Game_fmts.dir_to_string dir
              ^ ". Please try again.")
              adv st)
    | Quit ->
